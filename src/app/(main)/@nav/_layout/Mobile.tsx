...
import { BookOpenCheck, Compass, Gauge, MessageSquare, User } from 'lucide-react';
  ...
  const items: MobileTabBarProps['items'] = useMemo(
    () =>
      [
        ...
        {
          icon: (active: boolean) => (
            <Icon className={active ? styles.active : undefined} icon={Gauge} />
          ),
        ...
        {
          icon: (active: boolean) => (
            <Icon className={active ? styles.active : undefined} icon={BookOpenCheck} />
          ),
          key: SidebarTabKey.Query,
          onClick: () => {
            router.push('/query');
          },
          title: t('tab.chat'),
